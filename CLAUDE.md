...
P35: All-framework dashboard polish — overview `Run Assessment` section now dynamically shows buttons for every active framework instead of hardcoded SOC2/ISO27001; control library framework filter tabs now derived from actual loaded controls (all 10 supported); policy page `frameworkColor` covers all 10 frameworks; policies `triggerAssessment` uses org's primary active framework; audit history cycle modal includes all 10 frameworks; `ControlFramework` API type expanded to all 10; both TypeScript workspaces pass clean
