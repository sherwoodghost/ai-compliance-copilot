...
export default function AuditExportsPage() {
  ...
  async function downloadAuditPackage() {
    ...
    try {
      const blob = await auditApi.downloadAuditPackage();
      ...
      console.error('Audit package download failed:', err);
    } finally {
    ...
    <div className="p-8 max-w-5xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-600 text-white text-sm font-medium hover:bg-brand-700 transition-colors disabled:opacity-60"
            onClick={downloadAuditPackage}
            disabled={downloadingPackage}
            title="Download complete audit package as ZIP (policies, evidence, control matrix, risk register)"
          >
            {downloadingPackage
              ? <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Building…</>
              : <><Package className="w-3.5 h-3.5" />Download Audit Package</>
            }
          </button>
          <button
