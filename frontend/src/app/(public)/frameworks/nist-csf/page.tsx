...
export const metadata: Metadata = {
  title: 'NIST Cybersecurity Framework 2.0 Controls | ComplianceOS',
  description:
    'Browse all NIST CSF 2.0 controls organized by the six core Functions — Govern, Identify, Protect, Detect, Respond, and Recover.',
};

// Display order for NIST CSF 2.0 core Functions
...
export default async function NistCsfPage() {
  const controls = await getFrameworkControls('nist-csf').catch(() => [] as Control[]);
      ...
      <section className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-10">
          ...
          <nav className="flex items-center gap-1 text-xs text-gray-400 mb-4">
            ...
            <span className="text-gray-600 font-medium">NIST CSF</span>
          </nav>

          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              <span className="inline-block text-xs font-semibold bg-orange-100 text-orange-700 px-2.5 py-1 rounded-full mb-3">
                NIST Cybersecurity Framework 2.0
              </span>
              <h1 className="text-3xl font-bold text-gray-900">NIST CSF 2.0</h1>
              <p className="text-gray-500 mt-2 max-w-2xl leading-relaxed">
                The NIST Cybersecurity Framework provides a voluntary, risk-based approach to managing
                cybersecurity risk. Version 2.0 adds a new Govern function and expands applicability
                beyond critical infrastructure. The framework organizes {controls.length || 49} subcategory
                controls across {grouped.length || 6} core Functions.
              </p>
            </div>
            <div className="flex gap-4 text-center shrink-0">
              <div className="bg-gray-50 rounded-lg px-4 py-2 border border-gray-200">
                <p className="text-2xl font-bold text-gray-900">{controls.length || 49}</p>
                <p className="text-xs text-gray-500">Controls</p>
              </div>
              <div className="bg-gray-50 rounded-lg px-4 py-2 border border-gray-200">
                <p className="text-2xl font-bold text-gray-900">{grouped.length || 6}</p>
                <p className="text-xs text-gray-500">Core Functions</p>
              </div>
            </div>
          </div>
        </div>
